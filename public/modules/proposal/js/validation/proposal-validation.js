$(document).ready(function () {
  ...
  $('#proposal-form').validate({
    ...
    messages: {
      ...
      expired_at: { required: 'Vui lòng nhập hạn báo giá' },
      amount: { required: 'Vui lòng nhập giá trị đơn hàng' },
  ...
  function addServiceValidation(index) {
    const validator = $('#proposal-form').validate();
  ...
  $(document).on(
    ...
    function () {
      ...
      if ($(this).hasClass('select2-product')) {
        ...
        setTimeout(function() {
          $('#proposal-form').validate().element(priceInput);
  ...
  $(document).on(
    ...
    function () {
      ...
      if ($(this).hasClass('service-price')) {
        ...
        $('#proposal-form').validate().element(element);
  ...
  // Handle customer change - preserve validation state
  $(document).on('change', '#customer_select', function() {
    // Re-validate customer field to maintain validation state
    setTimeout(function() {
      $('#proposal-form').validate().element($('#customer_select'));
    }, 100);
  });
  ...
  $(document).on('invalid-form.validate', '#proposal-form', function() {
    setTimeout(restoreNumberFormat, 100);
  });

  $(document).on('valid-form.validate', '#proposal-form', function() {
    setTimeout(restoreNumberFormat, 100);
  });
