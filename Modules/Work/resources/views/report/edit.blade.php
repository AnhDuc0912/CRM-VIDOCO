    ...
    <div class="page-content">
            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Cập Nhật Báo Cáo</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-body p-4">
                ...
                <form action="{{ route('work.report.update', $report->id) }}" method="POST" enctype="multipart/form-data"
        ...
        {{ old('content', $report->content ?? '') }}
                    ...
                    <div class="col-12">
                        ...
                        @if ($report?->files && $report?->files && $report?->files->count() > 0)
                    ...
                    <div class="mb-3">
                        <label for="receiver_status" class="form-label">Trạng Thái</label>
                        <select name="receiver_status" id="receiver_status" class="form-select single-select" required>
                            @foreach (\Modules\Work\Models\WorkReport::STATUS_LABELS as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('receiver_status', $report->receiver_status) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
