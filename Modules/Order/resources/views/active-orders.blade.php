...
@section('title', 'Dịch vụ đang sử dụng')
        ...
        <div class="card-body">
            <div class="d-flex justify-content-between">
                <div class="card-title">
                    <h4 class="mb-0">Danh sách dịch vụ đang sử dụng</h4>
                </div>
                <div class="btn-group">
                    ...
                    <tbody>
                        @forelse ($orders as $order)
                            @foreach ($order->orderServices as $orderService)
                                @if (
                                    $orderService->end_date &&
                                        \Carbon\Carbon::parse($orderService->end_date)->isAfter(\Carbon\Carbon::now()) &&
                                        $orderService->status)
                                    <tr>
                                        ...
                                        <td>{{ $orderService->service->category->name ?? '' }}</td>
                                           <td>{{ $order->customer?->customer_type == CustomerTypeEnum::COMPANY ? $order->customer?->company_name : $order->customer?->last_name . ' ' . $order->customer?->first_name ?? '' }} - {{ $order->customer?->phone }}  </td>
                                        <td>
                                            S:
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center">Không có dịch vụ đang sử dụng</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
