...
@section('title', 'Dịch vụ đã hết hạn')
        ...
        <div class="card-body">
            <div class="d-flex justify-content-between">
                <div class="card-title">
                    <h4 class="mb-0">Danh sách dịch vụ đã hết hạn</h4>
                </div>
                <div class="btn-group">
                    ...
                    <tbody>
                        @forelse ($orders as $order)
                            @foreach ($order->orderServices as $orderService)
                                @if ($orderService->end_date && \Carbon\Carbon::parse($orderService->end_date)->isBefore(\Carbon\Carbon::now()))
                                    <tr>
                                        <td>{{ $orderService->id }}</td>
                                        <td>{{ $orderService->domain ?? '' }}</td>
                                        <td>{{ $orderService->service->category->name ?? '' }}</td>
                                        <td> {{ $order->customer->company_name ?? '' }} - {{ $order->customer->phone }}
                                        </td>
                                        <td>
                                            S:
                                        ...
                                        <td class="text-center">
                                            <div class="form-check form-center form-switch">
                                                <input class="form-check-input" type="checkbox"
                                                    {{ $orderService->status ? 'checked' : '' }} disabled>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox"
                                                    {{ $orderService->auto_email ? 'checked' : '' }} disabled>
                                            </div>
                                        </td>
                                        <td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center">Không có dịch vụ đã hết hạn</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
